...
@Component({
  selector: 'app-crazy',
  templateUrl: './crazy.page.html',
  styleUrls: ['./crazy.page.scss'],
})
export class CrazyPage implements OnInit {
...
async presentToast() {
  const toast = await this.toastController.create({
    message: 'New recipe has been added to your lunchbox!',
    duration: 2000
  });
  toast.present();
}
  ...
  ngOnInit() {
    this.db.collection<any>( "food", ref => ref.where ('status' , '==' ,"crazy").limit(1)).valueChanges ().subscribe((data)=>{
        this.data = data;
